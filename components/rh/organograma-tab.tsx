...
const STATUS_CORES: Record<string, string> = {
  ativo: "#27AE60", ferias: "#C2A878", licenca: "#2980B9", inativo: "#95A5A6",
};
...
function NodeCard({ node }: { node: OrgNode }) {
  return (
    <div className="flex flex-col items-center">
      {/* Card */}
      <div className="bg-surface border border-[#E0D0B4]/50 rounded-card px-4 py-3 text-center shadow-sm min-w-[140px] max-w-[180px] relative">
        <div
      ...
      {node.filhos.length > 0 && (
        <div className="flex flex-col items-center">
          ...
          <div className="w-px h-6 bg-[#E0D0B4]/60" />
          ...
          {node.filhos.length === 1 ? (
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="relative flex items-start">
                {/* Left cap */}
                <div
                  className="absolute top-0 border-t border-l border-[#E0D0B4]/60"
                  style={{
                    left: "50%",
                    width: `calc(50% - 70px)`,
                    height: "12px",
                ...
                {/* Right cap */}
                <div
                  className="absolute top-0 border-t border-r border-[#E0D0B4]/60"
                  style={{
                    right: "50%",
                    width: `calc(50% - 70px)`,
                    height: "12px",
              ...
              <div className="flex items-start gap-6 pt-3">
                {node.filhos.map((filho) => (
                  <div key={filho.id} className="flex flex-col items-center">
                    <div className="w-px h-3 bg-[#E0D0B4]/60" />
                    <NodeCard node={filho} />
                  </div>
                ))}
              </div>
...
export default function OrganogramaTab({ funcionarios }: { funcionarios: Funcionario[] }) {
    ...
    return (
      <div>
        ...
        <div className="flex flex-wrap gap-3">
          {funcionarios.map((f) => (
            <div key={f.id} className="bg-surface border border-[#E0D0B4]/50 rounded-card px-4 py-3 text-center shadow-sm min-w-[140px]">
              <div className="w-9 h-9 rounded-full bg-primary flex items-center justify-center mx-auto mb-2">
  ...
  return (
    <div className="overflow-x-auto pb-6">
      ...
      {/* Legenda */}
      <div className="flex items-center gap-4 mt-8 pt-4 border-t border-[#E0D0B4]/30 flex-wrap">
